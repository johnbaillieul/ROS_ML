...
from vbn_msgs.msg import TauComputation
import numpy as np
import time
import sys
...
class Controller(Node):
    ...
    def __init__(self):
        ...
        # Tau Data Subscriber
        self.tau_values = self.create_subscription(TauComputation, "/tau_values_of", self.callback, 10)
        ...
        self.sense_rate = 50
        self.sense_rate_ = self.create_rate(self.sense_rate)
        self.act_rate = 50
        self.act_rate_ = self.create_rate(self.act_rate)
    ...
    def callback(self, data):
        ...
        if self.sense:
            ...
            # Last time in sense phase
                ...
                tau_center_considered = self.tau_center_values[
                                        int(self.percentage * np.size(self.tau_center_values)):
        ...
        if self.act:
            ...
            if self.act_cnt <= (self.act_rate * self.act_duration):
                ...
                if not self.tau_diff_max:
                    if self.obstacle:
                        find_obstacle(self, self.mean_tau_center,
                                      self.time_to_obstacle)  # The obstacle is at 2 second?
                        if self.extreme_left and self.extreme_right:
